...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "professional prepping for a certification exam"
    - topic: "pain category"
      decision: "workflow friction — SRS itself is fine, manual card authoring is the bottleneck"
    - topic: "insight"
      decision: "existing tools are either too manual (Anki-style) or too simplistic (no real SRS algorithm); none solve the authoring bottleneck well with AI"
    - topic: "auth strategy"
      decision: "login via email + password"
    - topic: "role model"
      decision: "flat — all users equal, no admin role for MVP"
    - topic: "MVP flow scope"
      decision: "full flow committed for v1 (login, AI generation, manual CRUD, review/accept/edit/reject, SRS review) — 3 weeks after-hours, eyes-open re: two integrations (AI gen + SRS algorithm)"
  frs_drafted: 10
