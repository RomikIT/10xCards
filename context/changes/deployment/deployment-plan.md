...
- [ ] **Manual step required (human-only, dashboard access):** Supabase project's Authentication → URL Configuration → **Site URL** (and Redirect URLs allowlist) still points at the *old* URL (`https://10x-astro-starter.romanj23-f66.workers.dev`, set during Phase 5's fix) and must be updated to `https://10xcards.romanj23-f66.workers.dev` — otherwise the same "confirmation link redirects to an unreachable URL" failure from [[Set Supabase Auth's Site URL before trusting a production signup flow]] in `lessons.md` will recur.
- [ ] CI auto-deploy not yet re-verified under the new name — the `.github/workflows/ci.yml` deploy job needs no changes (it reads the Worker name from `wrangler.jsonc`), but a push to `master` should be done to confirm the pipeline deploys `10xcards` cleanly end-to-end.
- [ ] **Open decision for the user:** delete the old `10x-astro-starter` Worker (and its `10x-astro-starter-session` KV namespace) from the Cloudflare dashboard/CLI once the new one is confirmed working, to avoid confusion and stop it holding stale secrets — not done automatically since deleting a Worker is destructive.
